...
This is the dReal team's fork of [ibex-team/ibex-lib](https://github.com/ibex-team/ibex-lib) (Ibex 2.9.1) — a C++ interval-arithmetic / constraint-programming library. The fork exists **only** to host seven surgical patches that are pending upstream submission.
...
**Read [MIGRATION.md](MIGRATION.md) before making non-trivial changes.** It catalogs the 7 patches (what / why / file scope) and their upstream-PR status.

## Branches

- `master` — tracks `origin/master`. Do not commit local changes here.
- `dreal-perf-patches` — the 7-patch series (11 files / +163/−19 vs mainline, excluding docs). This is the branch `dreal4-cmake` should depend on.
...
If any of the 7 patches lands upstream, drop the corresponding commit from the rebase.
