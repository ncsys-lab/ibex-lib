...
void TestExprPolynomial::test00() {
	ExprPolynomial p;
	CPPUNIT_ASSERT(((std::string) p)=="0");
	CPPUNIT_ASSERT(((std::string) -p)=="0");
	CPPUNIT_ASSERT(((std::string) (p+p))=="0");
	CPPUNIT_ASSERT(((std::string) (p*p))=="0");
	p+=p;
	CPPUNIT_ASSERT(((std::string) p)=="0");
	p-=p;
	...
	ExprPolynomial p1(3);
	CPPUNIT_ASSERT(((std::string) p1)=="3");
	CPPUNIT_ASSERT(((std::string) (p+p1))=="3");
	CPPUNIT_ASSERT(((std::string) (p1+p1))=="6");
	CPPUNIT_ASSERT(((std::string) (p1-p1))=="0");
	CPPUNIT_ASSERT(((std::string) (p1*p1))=="9");
	CPPUNIT_ASSERT(((std::string) (p*p1))=="0");
	CPPUNIT_ASSERT(p1.is_constant());
	CPPUNIT_ASSERT(p1.one_monomial());
	CPPUNIT_ASSERT(sameExpr(p1.to_expr(),"3"));

	ExprPolynomial p2(4);
	CPPUNIT_ASSERT(((std::string) (p1+p2))=="7");
	CPPUNIT_ASSERT(((std::string) (p2-p1))=="1");
	CPPUNIT_ASSERT(((std::string) (p1*p2))=="12");
	p2+=p1;
	CPPUNIT_ASSERT(((std::string) p2)=="7");
	...
	CPPUNIT_ASSERT(((std::string) p1)=="3");
	p1+=p1;
	CPPUNIT_ASSERT(((std::string) p1)=="6");
}
...
void TestExprPolynomial::test01() {
	...
	CPPUNIT_ASSERT(((std::string) (2*p))=="2x");

	CPPUNIT_ASSERT(((std::string) (p+p))=="2x");

	CPPUNIT_ASSERT(sameExpr((p+p).to_expr(),"(2*x)"));

	CPPUNIT_ASSERT(((std::string) p.square())=="x^2");
	CPPUNIT_ASSERT(sameExpr(p.square().to_expr(),"x^2"));

	CPPUNIT_ASSERT(((std::string) (p*p))=="x^2");
	CPPUNIT_ASSERT((p*p).one_monomial());

	p*=2;
	CPPUNIT_ASSERT(((std::string) p)=="2x");
	CPPUNIT_ASSERT(p.one_monomial());

	p+=p;
	CPPUNIT_ASSERT(((std::string) p)=="4x");
...
void TestExprPolynomial::test02() {
	...
	CPPUNIT_ASSERT(((std::string) (px+py))=="x+y");
	CPPUNIT_ASSERT(sameExpr((px+py).to_expr(),"(x+y)"));

	CPPUNIT_ASSERT(((std::string) (px+Interval::one()))=="1+x");

	CPPUNIT_ASSERT(((std::string) ((px+py)-(px+py)))=="0");

	CPPUNIT_ASSERT(((std::string) (2*px+3*py))=="2x+3y");

	CPPUNIT_ASSERT(((std::string) (2*px-3*py))=="2x+-3y");

	CPPUNIT_ASSERT(((std::string) (2*(px+py)))=="2x+2y");

	CPPUNIT_ASSERT(((std::string) (px*py))=="xy");

	ExprPolynomial px_y2=(px+py)*(px+py);
	CPPUNIT_ASSERT(((std::string) px_y2)=="x^2+2xy+y^2");
	CPPUNIT_ASSERT(sameExpr(px_y2.to_expr(),"((x^2+(2*(x*y)))+y^2)"));
	CPPUNIT_ASSERT(((std::string) ((px+py)*(px-py)))=="x^2+-y^2");

	ExprPolynomial px_y=px+py;
	CPPUNIT_ASSERT(!px_y.is_constant());
	CPPUNIT_ASSERT(!px_y.one_monomial());

	CPPUNIT_ASSERT(((std::string) -px_y)=="-x+-y");
	CPPUNIT_ASSERT(sameExpr((-px_y).to_expr(),"((-x)-y)"));

	CPPUNIT_ASSERT(((std::string) px_y.square())=="x^2+2xy+y^2");

	px_y+=px;
	CPPUNIT_ASSERT(((std::string) px_y)=="2x+y");
...
void TestExprPolynomial::test03() {
	...
	CPPUNIT_ASSERT(((std::string) puv)=="(v.(u)')");
	CPPUNIT_ASSERT(sameExpr(puv.to_expr(),"((v)'*(u)')"));

	CPPUNIT_ASSERT(puv.one_monomial());
	CPPUNIT_ASSERT(((std::string) (puv+px))=="x+(v.(u)')");

	ExprPolynomial p1=puv+pw*pz+px+Interval::one();

	CPPUNIT_ASSERT(sameExpr(p1.to_expr(),"(((1+x)+((v)'*(u)'))+((z)'*(w)'))"));

	CPPUNIT_ASSERT(((std::string) p1)=="1+x+(v.(u)')+(z.(w)')");
	CPPUNIT_ASSERT(((std::string) (2*p1))=="2+2x+2(v.(u)')+2(z.(w)')");
	CPPUNIT_ASSERT(((std::string) (p1+p1))=="2+2x+2(v.(u)')+2(z.(w)')");
	CPPUNIT_ASSERT(((std::string) (p1-px))=="1+(v.(u)')+(z.(w)')");
	CPPUNIT_ASSERT(((std::string) (p1-px-Interval::one()))=="(v.(u)')+(z.(w)')");
	CPPUNIT_ASSERT(((std::string) (p1-px-pw*pz))=="1+(v.(u)')");
	CPPUNIT_ASSERT(((std::string) (p1-px-pw*pz-puv))=="1");
	CPPUNIT_ASSERT(((std::string) (p1-px-Interval::one()-pw*pz-puv))=="0");

	CPPUNIT_ASSERT(((std::string) (px*p1))=="x+x^2+x(v.(u)')+x(z.(w)')");

	CPPUNIT_ASSERT(sameExpr((px*p1).to_expr(),"(((x+x^2)+(x*((v)'*(u)')))+(x*((z)'*(w)')))"));

	CPPUNIT_ASSERT(((std::string) (p1*puv))=="x(v.(u)')+(v.(u)')+(v.(u)')^2+(v.(u)')(z.(w)')");
...
void TestExprPolynomial::test04() {
	...
	CPPUNIT_ASSERT(((std::string) (pxuvA*pxuvB))=="x^2(v.(u)')^2AIB");

	CPPUNIT_ASSERT(((std::string) (pxuvA+pxuvB))=="x(v.(u)')A+x(v.(u)')B");

	CPPUNIT_ASSERT(((std::string) (pxuvA+pxuvA))=="2x(v.(u)')A");

	CPPUNIT_ASSERT(((std::string) (pxuvA-pxuvA))=="0");

	CPPUNIT_ASSERT(((std::string) (pxuvA*pz))=="x(v.(u)')AIz");
	...
	CPPUNIT_ASSERT(((std::string) ((px*mw+px*mr)*(px*pz+pu*pv*mc+mc)))==ss.str());
